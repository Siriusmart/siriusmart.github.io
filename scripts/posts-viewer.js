...
function display(posts) {
  ...
  if (posts.length === 0) {
    ...
    if (page > 1) {
      ...
      first.onclick = () =>
        window.exit(`./posts.html?p=1&n=${number_per_page}`);
      ...
      back.onclick = () =>
        window.exit(`./posts.html?p=${page - 1}&n=${number_per_page}`);
    ...
    for (let i = start; i <= end; i++) {
      ...
      button.onclick = () =>
        window.exit(`./posts.html?p=${i}&n=${number_per_page}`);
    ...
    if (page < max_page) {
      ...
      last.onclick = () =>
        window.exit(`./posts.html?p=${max_page}&n=${number_per_page}`);
    ...
    window.createIframe(
      `${
        window.env.customFilesUrl
          ? window.env.customFilesUrl
          : window.env.filesUrl
      }/index.html?path=./posts/${
        posts[i]
      }.json&type=post-preview&label=${JSON.stringify({
        index: start + i,
        id: posts[i],
      })}`
    );
...
window.createIframe(
  ...
  }/index.html?path=./posts/index.json&type=posts-index`
);
